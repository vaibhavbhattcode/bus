  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket) {
    try {
      console.log(`[Gateway] New connection attempt from ${client.id}`);
      const token = this.extractToken(client);
      
      if (!token) {
        console.warn(`[Gateway] Connection attempt without token from ${client.id}`);
        client.disconnect();
        return;
      }

      // Log part of the token for debugging (security: do not log full token in prod)
      console.log(`[Gateway] Token received for ${client.id}: ${token.substring(0, 15)}...`);
      ...
      console.log(`[Gateway] User connected: ${userId} (Socket: ${client.id})`);
    } catch (e) {
      console.error(`[Gateway] Connection unauthorized for ${client.id}:`, e.message);
