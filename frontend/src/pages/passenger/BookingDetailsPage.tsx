...
export default function BookingDetailsPage() {
  ...
  const [qrTicket, setQrTicket] = useState<{ id: string, qrCode: string, signature: string } | null>(null);
  ...
  const handleDownloadTicket = () => {
    ...
    try {
      ...
      doc.setFontSize(10);
      doc.text(`Booking ID: ${booking.id.slice(0, 8).toUpperCase()}`, pageWidth - 14, 15, { align: 'right' } as any);
      doc.text(`Booked On: ${format(new Date(booking.createdAt), 'dd MMM yyyy')}`, pageWidth - 14, 20, { align: 'right' } as any);
      doc.text(`Status: ${booking.status}`, pageWidth - 14, 25, { align: 'right' } as any);
                ...
                >
                  ...
                  <div className="p-8 flex flex-col items-center bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-opacity-5">
                    <div className="bg-white p-4 rounded-3xl shadow-lg border border-gray-100 transform hover:scale-105 transition-transform duration-300">
                      <img src={qrTicket.qrCode} alt="Boarding Pass QR" className="w-56 h-56 object-contain" />
                    </div>
